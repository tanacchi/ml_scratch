...
class UKR(object):
    ...
    def fit(self, X, num_epoch=50):
        ...
        history = History(Y=np.empty((num_epoch, N, D)),
                          f=np.empty((num_epoch, f_resolution**self.L, D)),
                          Z=np.empty((num_epoch, N, self.L)))
